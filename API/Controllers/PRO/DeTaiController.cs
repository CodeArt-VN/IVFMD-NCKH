...
namespace API.Controllers.PRO
{
    ...
    public class DeTaiController : CustomApiController
    {
        ...
        public IHttpActionResult Print(DTO_PRO_DeTai_PrinterData item)
        {
            ...
            var pp = NckhHtmlToPdfConverter.HtmlToPdf(System.Web.Hosting.HostingEnvironment.MapPath(path), item.htmlContent, item.htmlHeader, item.htmlFooter, (int)item.pxHeader, (int)item.pxFooter, item.firstPageHeader);
            return Ok(path);
        }
