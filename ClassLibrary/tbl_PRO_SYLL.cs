...
namespace ClassLibrary
{
    ...
    public partial class tbl_PRO_SYLL
    {
...
namespace BaseBusiness
{
    ...
    {
		public static IQueryable<DTO_PRO_SYLL> toDTO(IQueryable<tbl_PRO_SYLL> query)
        {
			return query
			.Select(s => new DTO_PRO_SYLL(){							
				ID = s.ID,							
				IDDetai = s.IDDetai,							
				IDNhanSu = s.IDNhanSu,							
				HoTen = s.HoTen,							
		...
		public static bool put_PRO_SYLL(AppEntities db, int PartnerID, int ID, DTO_PRO_SYLL item, string Username)
        {
            ...
            {							
                ...
                try
                {
					...
					BS_CUS_Version.update_CUS_Version(db, null, "DTO_PRO_SYLL", dbitem.ModifiedDate, Username);
									
					result = true;
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("put_PRO_SYLL",e);
                    result = false;
                }
            }
            return result;
		...
		public static bool delete_PRO_SYLL(AppEntities db, int ID, string Username)
        {
			bool result = false;
            var dbitem = db.tbl_PRO_SYLL.Find(ID);
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("delete_PRO_SYLL",e);
                    result = false;
                }
            }
            return result;
		...
		public static bool check_PRO_SYLL_Exists(AppEntities db, int id)
		{
			return db.tbl_PRO_SYLL.Any(e => e.ID == id);
		}
