        ...
        public static DTO_PRO_DeTai get_PRO_DeTaiCustom(AppEntities db, int ID)
        {
            ...
            if (query != null)
            {
                ...
                if (db.tbl_PRO_BangKiemLuaChonQuyTrinhXXDD.Any(c => c.IDDeTai == ID && c.IsDeleted == false))
                    query.ListFormStatus.Add(new DTO_PRO_DeTai_TrangThai { Index = 14, Type = 1, Name = "Bảng kiểm xem xét đạo đức", Description = "Bảng kiểm xem xét đạo đức", FormCode = "tbl_PRO_BangKiemLuaChonQuyTrinhXXDD", TrangThai = "Đã tạo", TrangThaiCode = "Update" });
                else query.ListFormStatus.Add(new DTO_PRO_DeTai_TrangThai { Index = 14, Type = 1, Name = "Bảng kiểm xem xét đạo đức", Description = "Bảng kiểm xem xét đạo đức", FormCode = "tbl_PRO_BangKiemLuaChonQuyTrinhXXDD", TrangThai = "Chưa tạo", TrangThaiCode = "New" });
