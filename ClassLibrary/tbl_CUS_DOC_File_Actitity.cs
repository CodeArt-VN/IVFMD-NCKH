...
namespace BaseBusiness
{
    ...
    public static partial class BS_CUS_DOC_File_Actitity 
    {
        ...
        {
			var query = db.tbl_CUS_DOC_File_Actitity.Where(d => d.IsDeleted == false && d.IDPartner == PartnerID);
        ...
        {
            var dbResult = db.tbl_CUS_DOC_File_Actitity.FirstOrDefault(d => d.IsDeleted == false && d.IDPartner == PartnerID && d.Code == code);
			if (dbResult == null || dbResult.IDPartner != PartnerID)
				return null; 
			else 
				return toDTO(dbResult);
